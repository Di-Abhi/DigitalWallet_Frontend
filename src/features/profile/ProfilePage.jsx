...
import { User, Camera, FileText, CheckCircle, Clock, XCircle, Upload } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const submitKyc = async () => {
    ...
    try {
      ...
      const res = await userApi.kycSubmit(fd, kycForm.docType, kycForm.docNumber);
      ...
      <div className="card p-6">
        ...
        {kyc && kycStatus !== 'NOT_SUBMITTED' && (
          <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl mb-5 grid grid-cols-2 gap-3 text-sm">
            ...
            <div><span className="text-[var(--text-muted)]">Submitted: </span><span className="font-semibold">{kyc.submittedAt ? new Date(kyc.submittedAt).toLocaleDateString() : '—'}</span></div>
            {kyc.rejectionReason && (
        ...
        {(kycStatus === 'NOT_SUBMITTED' || kycStatus === 'REJECTED') && (
          <div className="space-y-4">
            <div>
              ...
              <select className="input-field" value={kycForm.docType} onChange={(e) => setKycForm({ ...kycForm, docType: e.target.value })}>
              ...
              <div className="border-2 border-dashed border-[var(--border)] rounded-xl p-6 text-center cursor-pointer hover:border-cyan-500 transition-colors"
                ...
                {kycFile ? (
                  <div className="flex items-center justify-center gap-2 text-emerald-500">
                    <CheckCircle className="w-5 h-5" /> <span className="text-sm font-semibold">{kycFile.name}</span>
                  </div>
                ) : (
                  <>
